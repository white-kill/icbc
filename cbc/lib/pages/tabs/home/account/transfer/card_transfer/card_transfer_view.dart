import 'package:cbc/pages/tabs/home/account/transfer/transfer_success/transfer_success_view.dart';
...
import 'conmponent/zh_info.dart';
import 'conmponent/card_input_money.dart';
import 'conmponent/zh_remarks.dart';

class CardTransferPage extends BaseStateless {
  CardTransferPage({Key? key}) : super(key: key, title: '境内汇款');
    ...
    return GetBuilder(
      builder: (CardTransferLogic c) {
        return ListView(
          ...
          children: [
            SizedBox(height: 6.w),
            logic.phoneTransfer ? const ZhInfo1():const ZhInfo(),
            SizedBox(height: 10.w),
            const CardInputMoney(),
            SizedBox(height: 10.w),
            const ZhRemarks(),
            GetBuilder(
              builder: (CardTransferLogic c) {
                        ...
                        Http.post(Apis.transfer,data: state.cardReq.toJson()).then((v){
                          if(v != null){
                            ...
                            Get.to(() => TransferSuccessPage(),arguments: {
                              'cardNo':state.cardReq.cardNo,
                              'bankName':state.cardReq.bankName,
                            });
                          }
                        });
                      });
